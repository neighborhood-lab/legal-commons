...
export function RegisteredAgentStep({
  initialData,
  state,
  onNext,
  onBack,
}: RegisteredAgentStepProps) {
  const {
    register,
    handleSubmit,
    watch,
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        ...
        <input
          id="agentName"
          type="text"
          className={`input ${errors.agentName ? 'input-error' : ''}`}
          {...register('agentName')}
        />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="md:col-span-2">
          <label htmlFor="agentStreet" className="label">
            Street Address
          </label>
          <input
            id="agentStreet"
            type="text"
            className={`input ${errors.agentStreet ? 'input-error' : ''}`}
            {...register('agentStreet')}
          />
        </div>
        <div>
          <label htmlFor="agentCity" className="label">
            City
          </label>
          <input
            id="agentCity"
            type="text"
            className={`input ${errors.agentCity ? 'input-error' : ''}`}
            {...register('agentCity')}
          />
        </div>
        <div>
          <label htmlFor="agentState" className="label">
            State
          </label>
          <input
            id="agentState"
            type="text"
            maxLength={2}
            className={`input ${errors.agentState ? 'input-error' : ''}`}
            {...register('agentState')}
          />
        </div>
        <div>
          <label htmlFor="agentZip" className="label">
            ZIP Code
          </label>
          <input
            id="agentZip"
            type="text"
            className={`input ${errors.agentZip ? 'input-error' : ''}`}
            {...register('agentZip')}
          />
        </div>
      </div>
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-6 mt-6">
        <label className="flex items-center mb-4">
          ...
          <span className="text-gray-900 dark:text-gray-100">
            Principal office is same as registered agent
          </span>
        </label>
        ...
        {!sameAsAgent && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label htmlFor="officeStreet" className="label">
            ...
            <div>
              <label htmlFor="officeCity" className="label">
                City
              </label>
              <input id="officeCity" type="text" className="input" {...register('officeCity')} />
            </div>
            <div>
              <label htmlFor="officeState" className="label">
                State
              </label>
              <input
                id="officeState"
            ...
            <div>
              <label htmlFor="officeZip" className="label">
                ZIP Code
              </label>
