...
export function MembersStep({ initialData, managementType, onNext, onBack }: MembersStepProps) {
  const [members, setMembers] = useState<Member[]>(
    initialData.members.length > 0 ? initialData.members : [{ name: '', email: '', ownershipPercentage: 100, isManager: true }]
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {members.map((member, index) => (
        <div key={index} className="border border-gray-300 dark:border-gray-600 rounded-lg p-4">
          <div className="flex justify-between items-center mb-4">
            ...
            {members.length > 1 && (
              <button type="button" onClick={() => removeMember(index)} className="text-error-600 hover:text-error-700">
      ...
      <div className={`p-4 rounded-lg ${totalOwnership === 100 ? 'bg-success-50 dark:bg-success-900/20' : 'bg-warning-50 dark:bg-warning-900/20'}`}>
        <p className="text-sm">
          Total Ownership: <strong>{totalOwnership.toFixed(2)}%</strong>
          {totalOwnership !== 100 && <span className="text-warning-700 dark:text-warning-400"> (Must equal 100%)</span>}
        </p>
      </div>

      <div className="flex justify-between pt-6">
        <button type="button" onClick={onBack} className="btn-secondary">Back</button>
        <button type="submit" className="btn-primary" disabled={totalOwnership !== 100}>Continue</button>
      </div>
    </form>
  );
}
